...
export default class EquipReservation extends Component {
  constructor(props) {
    super(props);
    this.equipValue = this.props.equipValue;
    this.parms = this.props.match.params;
    this.state = {
      equipInfo: null, // Place Information stored in DB
      date: new Date(),
      reservations: [],
      markedDates: []
    }
  ...
  async componentDidMount() {
    try {
      const existPlace = await axios.get(`${process.env.REACT_APP_API_URL}/equip/name/${this.parms.equipName}`);
      this.setState({
        equipInfo: existPlace.data
      })
      let currentDate = new Date();
      currentDate = currentDate.getFullYear() * 10000 + (currentDate.getMonth() + 1) * 100 + currentDate.getDate();
      const currentReservations = await axios.get(`${process.env.REACT_APP_API_URL}/reservation-equip/equipName/${this.parms.equipName}/${currentDate}`);
      this.setState({
        reservations: currentReservations.data
      })
      const existReservations = await axios.get(`${process.env.REACT_APP_API_URL}/reservation-equip/equipName/${this.parms.equipName}`);
      const markedDates = []
  ...
  handleDateChange = async (e, data) => {
    try {
      ...
      const existReservations = await axios.get(`${process.env.REACT_APP_API_URL}/reservation-equip/equipName/${this.parms.equipName}/${formedDate}`);
      this.setState({
        date: data.value,
    ...
    return (
      <section style={{padding: "0 1vw", margin: "0 2vw 4em"}}>
        <Grid columns={2} divided stackable>
          <Grid.Row>
            <Grid.Column width={6}>
              {this.state.equipInfo ?
                <Card fluid>
                  {
                    (this.state.equipInfo && this.state.equipInfo.imageName) ?
                      <Image src={`${process.env.REACT_APP_API_URL}/equip/image/${this.state.equipInfo.imageName}`}
                             wrapped ui={false}/>
                      : <Image src='https://react.semantic-ui.com/images/wireframe/image.png' wrapped ui={false}/>
                  }
                  <Card.Content>
                    <Card.Header>{this.state.equipInfo.name}</Card.Header>
                    <Card.Meta>
                      {this.state.equipInfo.location}
                    </Card.Meta>
                    <Card.Description>
                      {this.state.equipInfo.description}
                    </Card.Description>
                    <Card.Description style={{marginTop: "0.8em"}}>
                      {
                        this.state.userInfo ?
                          <EquipReservationCreateModal userInfo={this.state.userInfo} equipInfo={params}
                                                       trigger={
            ...
            <Grid.Column>
              <Grid rows={2} divided stackable>
                <Grid.Column>
                  ...
                  <Grid.Row>
                    <EquipReservationTable reservations={this.state.reservations}/>
                  </Grid.Row>
                </Grid.Column>
              </Grid>
            </Grid.Column>
